...
import n4.util.NDestroyUtil;

/**
 * A fairly generic quad tree structure for rapid overlap checks.
 * NQuadTree is also configured for single or dual list operation.
 ...
 * Ripped from HaxeFlixel: https://raw.githubusercontent.com/HaxeFlixel/flixel/master/flixel/system/FlxQuadTree.hx
 */
class NQuadTree extends NRect implements INDestroyable
{
	/**
	 ...
	 */
	private var _headA:NLinkedList;
	/**
	 * Refers to the internal A and B linked lists,
	 * which are used to store objects in the leaves.
	 */
	private var _tailA:NLinkedList;
	/**
	 * Refers to the internal A and B linked lists,
	 * which are used to store objects in the leaves.
	 */
	private var _headB:NLinkedList;
	/**
	 * Refers to the internal A and B linked lists,
	 * which are used to store objects in the leaves.
	 */
	private var _tailB:NLinkedList;
	 ...
	 * Internal, governs and assists with the formation of the tree.
	 */
	private var _northWestTree:NQuadTree;
	/**
	 * Internal, governs and assists with the formation of the tree.
	 */
	private var _northEastTree:NQuadTree;
	/**
	 * Internal, governs and assists with the formation of the tree.
	 */
	private var _southEastTree:NQuadTree;
	/**
	 * Internal, governs and assists with the formation of the tree.
	 */
	private var _southWestTree:NQuadTree;
	/**
	 * Internal, governs and assists with the formation of the tree.
	 */
	 ...
	 */
	private static var _object:NEntity;
	/**
	 * Internal, used to reduce recursive method parameters during object placement and tree formation.
	 */
	 ...
	 * Internal, used during tree processing and overlap checks.
	 */
	private static var _processingCallback:NEntity->NEntity->Bool;
	/**
	 * Internal, used during tree processing and overlap checks.
	 */
	private static var _notifyCallback:NEntity->NEntity->Void;
	/**
	 * Internal, used during tree processing and overlap checks.
	 */
	private static var _iterator:NLinkedList;
	...
	/**
	 * Pooling mechanism, turn NQuadTree into a linked list, when NQuadTrees are destroyed, they get added to the list, and when they get recycled they get removed.
	 */
	public static  var _NUM_CACHED_QUAD_TREES:Int = 0;
	private static var _cachedTreesHead:NQuadTree;
	private var next:NQuadTree;
	
	/**
	 * Private, use recycle instead.
	 */
	private function new(X:Float, Y:Float, Width:Float, Height:Float, ?Parent:NQuadTree)
	{
		super();
	 ...
	 */
	public static function recycle(X:Float, Y:Float, Width:Float, Height:Float, ?Parent:NQuadTree):NQuadTree
	{
		if (_cachedTreesHead != null)
		{
			var cachedTree:NQuadTree = _cachedTreesHead;
			...
			return new NQuadTree(X, Y, Width, Height, Parent);
	}
	/**
	...
	public function reset(X:Float, Y:Float, Width:Float, Height:Float, ?Parent:NQuadTree):Void
	{
		exists = true;
		
		set(X, Y, Width, Height);
		
		_headA = _tailA = NLinkedList.recycle();
		_headB = _tailB = NLinkedList.recycle();
		
		//Copy the parent's children (if there are any)
		if (Parent != null)
		{
			var iterator:NLinkedList;
			var ot:NLinkedList;
	...
	{
		_headA = NDestroyUtil.destroy(_headA);
		_headB = NDestroyUtil.destroy(_headB);

		_tailA = NDestroyUtil.destroy(_tailA);
		_tailB = NDestroyUtil.destroy(_tailB);

		_northWestTree = NDestroyUtil.destroy(_northWestTree);
		_northEastTree = NDestroyUtil.destroy(_northEastTree);
		
		_southWestTree = NDestroyUtil.destroy(_southWestTree);
		_southEastTree = NDestroyUtil.destroy(_southEastTree);
	 ...
	 * @param ObjectOrGroup1	Any object that is or extends NEntity or NGroup.
	 * @param ObjectOrGroup2	Any object that is or extends NEntity or NGroup.  If null, the first parameter will be checked against itself.
	 * @param NotifyCallback	A function with the form myFunction(Object1:NEntity,Object2:NEntity):void that is called whenever two objects are found to overlap in world space, and either no ProcessCallback is specified, or the ProcessCallback returns true. 
	 * @param ProcessCallback	A function with the form myFunction(Object1:NEntity,Object2:NEntity):Boolean that is called whenever two objects are found to overlap in world space.  The NotifyCallback is only called if this function returns true.  See NEntity.separate(). 
	 */
	public function load(ObjectOrGroup1:NBasic, ?ObjectOrGroup2:NBasic, ?NotifyCallback:NEntity->NEntity->Void, ?ProcessCallback:NEntity->NEntity->Bool):Void
	{
	 ...
	 * @param	ObjectOrGroup	NEntitys are just added, NGroups are recursed and their applicable members added accordingly.
	 * @param	List			A int flag indicating the list to which you want to add the objects.  Options are A_LIST and B_LIST.
	 */
	@:access(n4.group.NTypedGroup.resolveGroup)
	public function add(ObjectOrGroup:NBasic, list:Int):Void
	{
		_list = list;
		
		var group = NTypedGroup.resolveGroup(ObjectOrGroup);
		if (group != null)
		{
			var i:Int = 0;
			var basic:NBasic;
			...
			while (i < l)
			{
				...
				{
					group = NTypedGroup.resolveGroup(basic);
					if (group != null)
					{
						add(group, list);
					}
					...
					{
						_object = cast basic;
						if (_object.exists && _object.allowCollisions != NEntity.NONE)
						{
	...
	private function addObject():Void
	{
		...
		if ((_objectLeftEdge > _leftEdge) && (_objectRightEdge < _midpointX))
		{
			if ((_objectTopEdge > _topEdge) && (_objectBottomEdge < _midpointY))
			{
				...
				{
					_northWestTree = NQuadTree.recycle(_leftEdge, _topEdge, _halfWidth, _halfHeight, this);
				}
			...
			if ((_objectTopEdge > _midpointY) && (_objectBottomEdge < _bottomEdge))
			{
				...
				{
					_southWestTree = NQuadTree.recycle(_leftEdge, _midpointY, _halfWidth, _halfHeight, this);
				}
		...
		if ((_objectLeftEdge > _midpointX) && (_objectRightEdge < _rightEdge))
		{
			if ((_objectTopEdge > _topEdge) && (_objectBottomEdge < _midpointY))
			{
				...
				{
					_northEastTree = NQuadTree.recycle(_midpointX, _topEdge, _halfWidth, _halfHeight, this);
				}
			...
			if ((_objectTopEdge > _midpointY) && (_objectBottomEdge < _bottomEdge))
			{
				...
				{
					_southEastTree = NQuadTree.recycle(_midpointX, _midpointY, _halfWidth, _halfHeight, this);
				}
	...
	/**
	 * NQuadTree's other main function.  Call this after adding objects
	 * using NQuadTree.load() to compare the objects that you loaded.
